...
X = data_frame.drop(['Date','GLD'],axis=1)  #axis = 0은 행, 1은 열 ,'EUR/USD','USO'
Y = data_frame['GLD']
...
X_train_scaled = std_scaler.transform(train_X)
X_test_scaled = std_scaler.transform(test_X)


DT = DecisionTreeRegressor(random_state=2)
DT.fit(X_train_scaled, train_Y)  # 학습용 문제, 학습용 정답
score1 = DT.score(X_test_scaled, test_Y)  # 시험 문제, 시험 정답
...
score2 = RF.score(X_test_scaled, test_Y)

# [3] K근접이웃 예측기 머신러닝 알고리즘
KN = KNeighborsRegressor(n_neighbors=2)
KN.fit(X_train_scaled, train_Y)
score3 = KN.score(X_test_scaled, test_Y)

# [4] 선형회귀 머신러닝 알고리즘
LR = LinearRegression()
LR.fit(X_train_scaled, train_Y)
score4 = LR.score(X_test_scaled, test_Y)

# plt.plot(['DT', 'RF', 'K-NN', 'LR'], [score1, score2, score3, score4])
print('테스트 스코어: {0:.2f}, {1:.2f}, {2:.2f}, {3:.2f}'.format(score1, score2, score3, score4))
# plt.show()
